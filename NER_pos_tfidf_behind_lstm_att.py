...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, pos_to_ix, embedding_dim, hidden_dim,num_layers=2,method='ATTN_TYPE_DOT_PRODUCT'):
        ...
        self.lstmhidden2tag = nn.Linear(hidden_dim, self.tagset_size)
    ...
    def _get_lstm_features(self, sentence, pos_tagging,tf_idf):
        ...
        att_out = self._cal_attention(combined,self.method)
    ...
    def neg_log_likelihood(self, sentence, pos_tagging,tf_idf, tags):
        ...
        #attention_result = self._cal_attention(lstm_out, self.method)
        #attention_feats = self.hidden2tag(attention_result)
        lstm_feats = self.lstmhidden2tag(lstm_out)
